using UnityEngine;
using ChargerWirelessCharging_BZ.Patches;

namespace ChargerWirelessCharging_BZ.Mono
{
    public class WirelessChargerMonoObject : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (!ChargerWirelessCharging_BZ.Config.Config_modEnable) return;
            if (!basefound) return;
            if (interrimdistance > ChargerWirelessCharging_BZ.Config.Config_maxPlayerDistanceToCharger) return;
            ...
            if (wirelesschargertimer <= 0f)
            {
                ...
                if (powerRelay != null)
                {
                    ...
                    foreach (Battery battery in Player_BatteryStorage.batteries)
                    {
                        ...
                        if (charge < capacity)
                        {
                            float num3 = DayNightCycle.main.deltaTime * (charger.chargeSpeed/100*ChargerWirelessCharging_BZ.Config.Config_WirelessChargeSpeed) * capacity;
